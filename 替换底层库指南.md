...
- `libcore/bin/libcore.dylib` - macOS 核心库（预编译）
- `Makefile` - 项目根目录的构建脚本
...
⚠️ **注意**：macOS 核心库是预编译的，替换底层库后需要：

1. 如果有 macOS 编译支持，可以编译自定义库：
   ```bash
   cd libcore
   go mod tidy
   # 需要添加 macOS 编译目标到 Makefile
   make macos  # 如果已实现
   ```

2. 或者使用预编译的库（从 GitHub Releases 下载）：
   ```bash
   make macos-libs
   ```
...
# 检查文件大小（应该有几 MB）
du -h libcore/bin/libcore.dylib
```
...
macOS 核心库是预编译的 universal 二进制文件，包含 arm64 和 x86_64 架构。文件大小通常在 30-50MB 左右，这是正常的。
...
4. **备份**：替换前建议备份原始的库文件（AAR、Framework 或 macOS 核心库）
5. **Git 管理**：建议将修改后的 `go.mod` 提交到版本控制
6. **平台差异**：
   - Android 和 iOS 使用相同的 Go 代码，但编译产物不同（AAR vs Framework）
   - macOS 使用预编译的核心库，通常不需要从源码编译
